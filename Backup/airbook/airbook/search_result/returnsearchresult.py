...
class ReturnResult(GetElementPath):
    ...
    def check_book_link(self, book_link, decoded):
        print book_link,
        if book_link not in decoded:
            book_link = "not found"
        else:
            book_link = "found"
        return book_link
    ...
    def get_search_result(self, daydiff, input_prom_cod="none", current_depature_id=0, current_return_id=0):
        ...
        # if 365 <= daydiff <= 730 and input_prom_cod == "none":
        if 365 <= daydiff and input_prom_cod == "none":
            ...
            if (no_seats not in decoded) and (book_now not in decoded):
                ...
                self.driver.save_screenshot(actual_screen)
                print self.check_book_link(book_link, decoded)
        ...
        if 365 <= daydiff and input_prom_cod != "none":
            ...
            if book_now in decoded:
                ...
                self.driver.save_screenshot(actual_screen)
                print self.check_book_link(book_link, decoded)
                # print "valid daydiff & prom_cod, failed (cannot find expected text)", daydiff, input_prom_cod
                self.driver.back()
                return actual_screen, ret_validation, status
        else:
