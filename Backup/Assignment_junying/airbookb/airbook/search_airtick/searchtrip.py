...
class SearchTrip(GetElementPath):
    ...
    def get_depature_return(self, current_depature_id=0, current_return_id=0, input_prom_cod="none"):
        ...
        if input_prom_cod != "none":
            ...
            diffdate = day_diff_cal(current_depature_id, current_return_id)
            test_case_screen, expected, status = sr.get_search_result(diffdate, input_prom_cod, current_depature_id, current_return_id)
        else:
            current_depature = departure_selected.first_selected_option.text
            current_return = return_selected.first_selected_option.text
            diffdate = day_diff_cal(current_depature_id, current_return_id)
            test_case_screen, expected, status = sr.get_search_result(diffdate, input_prom_cod, current_depature_id, current_return_id)
        return (
            current_depature_id, current_return_id, current_depature, current_return, input_prom_cod, diffdate, test_case_screen, expected,
            status)